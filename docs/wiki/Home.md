...
A Bun-based bot that monitors public status pages — [Statuspage.io](https://www.atlassian.com/software/statuspage), [incident.io](https://incident.io/), and [Instatus](https://instatus.com/) are supported — and posts incident updates as threaded conversations in **Discord or Slack**.

One deployment drives one platform, chosen with `PLATFORM`. Every feature works the same on either.

## What It Does

- **Polls** one or more public status pages on a configurable interval (auto-detects provider)
- **Creates a thread** per incident, posting updates as they arrive
...
| [Slack Setup](Slack-Setup.md) | Slack app manifest, scopes, tokens, and Discord/Slack differences |
